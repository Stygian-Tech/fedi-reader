...
- **Label and metadata requirements for agent-found issues**: If the issue was discovered by an agent while working another task, apply the `Agent` label in MCP calls. In Linear UI this may appear under the `Source` group. Also carry over the relevant inferred metadata from the parent context whenever applicable, including product/platform labels, project association, priority, and any other labels that clearly match the affected area.
